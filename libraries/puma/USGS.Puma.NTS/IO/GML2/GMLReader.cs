...
namespace USGS.Puma.NTS.IO.GML2
{
    /// <summary>
    ...
    public class GMLReader
    {
        ...
        {
            bool hasStartElementGML = false;
            IGeometry result = null;

            // Read the first child and determine if it is a GML start element ...
            reader.Read();
            hasStartElementGML = IsStartElementGML(reader);

            // If the first child is a GML start element, then (1) read another child to get to the start
            // element of for a geometry, and (2) set the flag to read the end element for the GML tag when
            // everything is done.
            if (hasStartElementGML) reader.Read();

            // ... then seek for geometry element
            result = ReadGeometry(reader);

            // If a GML start element was read, then read the GML end element.
            if (hasStartElementGML) reader.Read();
        ...
        /// <param name="reader"></param>
        /// <returns></returns>
        public IGeometry ReadGeometry(XmlTextReader reader)
        {
            // Read and create the appropriate geometry
        ...
        /// <param name="reader"></param>
        /// <returns></returns>
        public bool IsGeometryElement(XmlTextReader reader)
        {
            if (reader.IsStartElement("Point", GMLElements.gmlNS))
                return true;
            else if (reader.IsStartElement("LineString", GMLElements.gmlNS))
        ...
        public bool IsStartElementGML(XmlTextReader reader)
        {
            return reader.IsStartElement("GML", GMLElements.gmlNS);
        }
