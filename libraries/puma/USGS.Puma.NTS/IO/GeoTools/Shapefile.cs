...
namespace USGS.Puma.NTS.IO
{
	...
	public class Shapefile
	{
		...
		{
            // All geometry types are implicitly treated as 3D shape features
            if (geom is IPoint)
                return ShapeGeometryTypes.PointZ;
            if (geom is IPolygon)
                return ShapeGeometryTypes.PolygonZ;
            if (geom is IMultiPolygon)
                return ShapeGeometryTypes.PolygonZ;
            if (geom is ILineString)
                return ShapeGeometryTypes.LineStringZ;
            if (geom is IMultiLineString)
                return ShapeGeometryTypes.LineStringZ;
            if (geom is IMultiPoint)
                return ShapeGeometryTypes.MultiPointZ;
		...
		{			
            using (SqlConnection connection = new SqlConnection(connectionstring))
            {
                ...
                DataTable shpDataTable = USGS.Puma.NTS.IO.Shapefile.CreateDataTable(filename, tableName, geometryFactory);
