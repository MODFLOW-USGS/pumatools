			...
			private ArrayList Read()  
			{
				...
				while (!foundRecord) 
				{
					...
					{                
                        ...
                        int decimalCount = _header.Fields[j].DecimalCount;
						...
						switch (tempFieldType)
						{
							...
							case 'F': // floating point number
								...
								try 
								{
                                    if (tempFieldType == 'N' && decimalCount == 0 && tempFieldLength < 10)
                                    {
                                        tempObject = int.Parse(tempString.Trim(), CultureInfo.InvariantCulture);
                                    }
                                    else
                                    {
