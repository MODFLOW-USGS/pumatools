...
namespace USGS.Puma.NTS.IO
{
    ...
    public class ShapefileDataWriter
    {
        ...
        public static DbaseFileHeader GetHeader(Feature feature, int count)
        {
            ...
            foreach (string name in names)
            {
                ...
                if (type == typeof(double) || type == typeof(float))
                    header.AddColumn(name, 'F', DoubleLength, DoubleDecimals);
                else if (type == typeof(short) || type == typeof(ushort) ||
                         type == typeof(int) || type == typeof(uint) ||
                         type == typeof(long) || type == typeof(ulong))
                {
                    double value = Convert.ToDouble(attribs[name]);
                    if (value >= 1000000000 || value <= -99999999)
                    {
                        header.AddColumn(name, 'N', 10, IntDecimals);
                    }
                    else
                    {
        ...
        private const int DoubleLength = 19;
        private const int DoubleDecimals = 11;
        private const int IntLength = 9;
