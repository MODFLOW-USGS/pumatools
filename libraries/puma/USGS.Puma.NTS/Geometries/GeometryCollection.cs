...
namespace USGS.Puma.NTS.Geometries
{
    ...
    public class GeometryCollection : Geometry, IGeometryCollection
    {
        ...
        {
            Geometry g;
            g = geometries[0] as Geometry;
            IVerticalEnvelope ve = new VerticalEnvelope(g.VerticalEnvelopeInternal.MinZ, g.VerticalEnvelopeInternal.MaxZ);
            for (int i = 1; i < geometries.Length; i++)
            {
                g = geometries[i] as Geometry;
                ve.ExpandToInclude(g.VerticalEnvelopeInternal);
            }
            return ve;
        }
