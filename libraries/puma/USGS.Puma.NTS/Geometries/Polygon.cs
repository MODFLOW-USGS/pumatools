...
namespace USGS.Puma.NTS.Geometries
{
    ...
    public class Polygon : Geometry, IPolygon
    {
        ...
        /// </summary>
        /// <returns></returns>
        protected override IVerticalEnvelope ComputeVerticalEnvelopeInternal()
        {
            if (IsEmpty)
                return new VerticalEnvelope() as IVerticalEnvelope;

            ICoordinate[] coords = this.Coordinates;
            double minZ = coords[0].Z;
            double maxZ = coords[0].Z;

            for (int i = 1; i < this.Coordinates.Length; i++)
            {
                minZ = minZ < coords[i].Z ? minZ : coords[i].Z;
                maxZ = maxZ > coords[i].Z ? maxZ : coords[i].Z;
            }

            return new VerticalEnvelope(minZ, maxZ) as IVerticalEnvelope;
